  ...
  "ProductionFrontendGlobal" must {
    "let requests passing" when {
      ...
      "coming from a IP NOT in the white-list and not with a white-listed path must be redirected" in {
        val request = FakeRequest(GET, "/sic-search/hello-world").withHeaders("True-Client-IP" -> "93.00.33.33")
        val Some(result) = route(app, request)

        status(result) mustBe SEE_OTHER

        redirectLocation(result) mustBe Some("https://www.tax.service.gov.uk/outage-sic-search")
      }
      ...
      "coming without an IP header must fail" in {
        val request = FakeRequest(GET, "/sic-search")
        val Some(result) = route(app, request)
